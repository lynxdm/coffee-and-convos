...
function Blog() {
  const { articles, isLoading, error, pinnedArticle } = useGetArticles(
  ...
  useEffect(() => {
    if (!isLoading && pinnedArticle.exists) {
      fetchArticleContent(pinnedArticle.article.id, "articles").then((data) => {
        ...
        <main className='px-6 lg:px-32'>
          {pinnedArticle.exists && (
            <section className='mt-8 flex flex-col items-center justify-between gap-4 border-b-2 border-primary pb-4 dark:border-[#3a3a3a] lg:flex-row lg:gap-8 lg:border-2 lg:pb-0'>
              <img
                src={pinnedArticle.article.cover.image}
                alt={pinnedArticle.article.title + "cover image"}
              ...
              <div className='flex flex-col gap-4 border-primary px-3 py-2 dark:border-[#3a3a3a] lg:gap-4 lg:px-0'>
                <Link
                  to={`/blog/${formatLink(pinnedArticle.article.title)}-${pinnedArticle.article.id}`}
                ...
                <div className='flex flex-col justify-center gap-1'>
                  ...
                  <p>{timeAgo(pinnedArticle.article.date)}</p>
                </div>
              </div>
            </section>
          )}
